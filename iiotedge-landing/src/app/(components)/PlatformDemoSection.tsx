...
import {
  ...
  Activity, Gauge, Thermometer, Power,
  Search, Filter, Download, Play, Pause,
  ChevronRight, Cpu, Cloud
} from 'lucide-react';
...
// ---- Module-level components (stable identity → child state survives parent re-renders) ----
    ...
    <div className="flex items-center justify-between mb-6">
      <div className="flex items-center space-x-4">
        ...
        <button
          ...
          aria-label={isPlaying ? 'Pause live data' : 'Resume live data'}
      ...
      <div className="flex items-center space-x-2">
        <button type="button" aria-label="Search" className="p-2 bg-slate-800 hover:bg-slate-700 rounded-lg transition-colors">
          <Search className="w-4 h-4 text-slate-400" />
        </button>
        <button type="button" aria-label="Filter" className="p-2 bg-slate-800 hover:bg-slate-700 rounded-lg transition-colors">
          <Filter className="w-4 h-4 text-slate-400" />
        </button>
        <button type="button" aria-label="Download" className="p-2 bg-slate-800 hover:bg-slate-700 rounded-lg transition-colors">
...
function DeviceCard({ device, isSelected, onClick }: DeviceCardProps) {
  return (
    <button
      type="button"
      onClick={onClick}
      className={`w-full text-left bg-slate-900/50 border rounded-xl p-4 cursor-pointer transition-all duration-300 ease-out ${
    ...
    >
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center space-x-3">
          <div className={`w-10 h-10 rounded-lg flex items-center justify-center ${
            ...
            <Cpu className={`w-5 h-5 ${device.status === 'online' ? 'text-green-400' : 'text-yellow-400'}`} />
          </div>
          <div>
      ...
      <div className="grid grid-cols-3 gap-3">
        <div>
          ...
          <div className="text-sm font-semibold text-white">{device.temp}&deg;F</div>
        </div>
        <div>
          <div className="flex items-center space-x-1 mb-1">
...
function LiveChart({ isPlaying }: LiveChartProps) {
  // Deterministic initial values for SSR hydration safety.
  const [data, setData] = useState<ChartDataPoint[]>(() =>
    Array.from({ length: 20 }, (_, i) => ({ time: i, value: 50 + (i % 10) * 3 }))
  );
  const [isMounted, setIsMounted] = useState(false);

  useEffect(() => {
    setIsMounted(true);
    setData(Array.from({ length: 20 }, (_, i) => ({ time: i, value: 50 + Math.random() * 30 })));
  }, []);
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      setData((prev) => [
        ...prev.slice(1),
        { time: prev[prev.length - 1].time + 1, value: 50 + Math.random() * 30 },
      ]);
    }, 1000);
...
function MonitoringView({
  ...
  return (
    <div className="space-y-6">
      <DashboardHeader isPlaying={isPlaying} onTogglePlay={onTogglePlay} />

      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <MetricCard icon={Cpu} label="Active Devices" value={metrics.activeDevices} change="+2.4%" positive={true} />
        <MetricCard icon={Database} label="Data Points/min" value={metrics.dataPoints.toLocaleString()} change="+12.3%" positive={true} />
        <MetricCard icon={Activity} label="System Uptime" value={`${metrics.uptime}%`} change="↑ 0.02%" positive={true} />
        <MetricCard icon={Bell} label="Active Alerts" value={metrics.alerts} change="2 resolved" positive={true} />
      </div>
...
function AnalyticsView({ isPlaying, onTogglePlay }: { isPlaying: boolean; onTogglePlay: () => void }) {
  return (
    <div className="space-y-6">
      <DashboardHeader isPlaying={isPlaying} onTogglePlay={onTogglePlay} />
        ...
        <div className="bg-slate-900/50 border border-slate-800 rounded-xl p-6">
          ...
          <div className="space-y-6">
            <div>
              ...
              <div className="text-sm text-green-400">&darr; 12% vs yesterday</div>
            </div>

            <div className="grid grid-cols-2 gap-4">
              <div className="bg-slate-800/50 rounded-lg p-4">
                <div className="text-xs text-slate-400 mb-1">Peak Hours</div>
                <div className="text-lg font-bold text-white">14:00&ndash;16:00</div>
              </div>
              <div className="bg-slate-800/50 rounded-lg p-4">
...
function AlertsView({ isPlaying, onTogglePlay }: { isPlaying: boolean; onTogglePlay: () => void }) {
  return (
    <div className="space-y-6">
      <DashboardHeader isPlaying={isPlaying} onTogglePlay={onTogglePlay} />
...
function ControlView({ isPlaying, onTogglePlay }: { isPlaying: boolean; onTogglePlay: () => void }) {
  return (
    <div className="space-y-6">
      <DashboardHeader isPlaying={isPlaying} onTogglePlay={onTogglePlay} />
          ...
          {DEVICES.slice(0, 4).map((device) => (
            <div key={device.id} className="bg-slate-800/50 border border-slate-700 rounded-xl p-6">
              <div className="flex items-center justify-between mb-4">
                ...
                <button type="button" aria-label="Device settings" className="p-2 bg-blue-500/10 hover:bg-blue-500/20 rounded-lg transition-colors duration-300 ease-out">
              ...
              <div className="space-y-3">
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex items-center space-x-2">
                    <input
                      ...
                      aria-label={`Power level for ${device.name}`}
                      className="w-24 h-1"
                    />
                ...
                <div className="flex items-center justify-between pt-3 border-t border-slate-700">
                  <button type="button" className="px-4 py-2 bg-green-500/10 hover:bg-green-500/20 border border-green-500/30 rounded-lg text-green-400 text-sm font-semibold transition-colors duration-300 ease-out">
                    Start
                  </button>
                  <button type="button" className="px-4 py-2 bg-yellow-500/10 hover:bg-yellow-500/20 border border-yellow-500/30 rounded-lg text-yellow-400 text-sm font-semibold transition-colors duration-300 ease-out">
                    Pause
                  </button>
                  <button type="button" className="px-4 py-2 bg-red-500/10 hover:bg-red-500/20 border border-red-500/30 rounded-lg text-red-400 text-sm font-semibold transition-colors duration-300 ease-out">
                    Stop
                  </button>
                </div>
              </div>
...
const PlatformDemoSection = () => {
  ...
  return (
    <section id="demo" ref={sectionRef} className="relative bg-slate-950 py-24 overflow-hidden">
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-6">
        <div className="text-center mb-16">
          ...
          >
            ...
            your industrial operations&mdash;all in one intuitive dashboard.
          </p>
        </div>
        ...
        <div className="bg-slate-900/30 backdrop-blur-sm border border-slate-800 rounded-2xl p-6 lg:p-8 mb-12">
          ...
          {activeTab === 'analytics' && <AnalyticsView isPlaying={isPlaying} onTogglePlay={togglePlay} />}
          {activeTab === 'alerts' && <AlertsView isPlaying={isPlaying} onTogglePlay={togglePlay} />}
          {activeTab === 'control' && <ControlView isPlaying={isPlaying} onTogglePlay={togglePlay} />}
        </div>
